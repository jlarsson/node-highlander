(function(module) {
  ...
  *************************************************/
  var Repository = (function RepositoryModule() {
    function exec(repo, entry) {
      return repo.commands.getCommand(entry.c)
        .bind({})
        .then(function exec_command(c) {
          this.command = c;
        })
        .then(function exec_log() {
          return entry.replay ? null : repo.journal.log(entry.c, entry.a);
        })
        .then(function exec_execute() {
          debug('executing %j', entry)
          entry.model = repo.model;
          return this.command.execute(entry);
        });
    }
    return classBuilder()
      ...
      .method('execute', function(commandName, argument, callback) {
        var self = this;
        return this
          .initialize()
          .then(function execute_run() {
            return Promise.using(self.lock.getWriteLock(commandName), function execute_scope() {
              return exec(self, {
                  c: commandName,
                  a: argument
                })
  ...
  *******************************************************************************/
  var Commands = (function CommandsModule() {
    return classBuilder()
      ...
      .method('setCommand', Promise.method(function setCommand(options, callback) {
        var command = Command(options);
        return createResolved(this.handlers[command.name] = command).nodeify(callback);
      }))
      .method('getCommand', Promise.method(function getCommand(name, callback) {
  ...
  *****************************************************/
  var Command = (function CommandModule() {
  ...
  *****************************************************/
  var FileJournal = (function() {
    ...
    function replayValidate(replayHandler, line) {
      ...
      if (line.indexOf('{') === 0) {
        // assume json
        return replayHandler(JSON.parse(line));
      }
      if (line === '') {
    ...
    return classBuilder()
      ...
      .method('log', Promise.method(function log(commandName, args, callback) {
